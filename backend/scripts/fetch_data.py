...
from app.seasons import current_season_year, season_years
...
def main():
    ...
    if not fetcher.api_key:
        raise ValueError("FOOTBALL_DATA_API_KEY must be configured before syncing")
    db = get_session_local()()

    try:
        total_added = 0
        for season in reversed(season_years()):
            ...
            try:
                ...
                if e.response is not None and e.response.status_code == 403:
                    if season == current_season_year():
                        raise RuntimeError("The provider denied access to the current season; sync is incomplete") from e
                    logger.warning(
